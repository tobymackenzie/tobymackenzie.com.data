...
Cell phone device would be composed of multiple standardized, purpose specific components / modules that could be replaced individually, added, or removed.  Modules would be built to and use an open communication standard used by all manufacturers.  Each module would be the width and depth of a normal cell phone, with one or a few open standards, but the height it needs to be to fit its contents.  Each except the end one would have a standard male USB C connector on one side and a female USB C on the other, such that each can connect to the next module.  The end one would just have a female connector so that there isn't a male one sticking out.  Each USB C port would support power delivery, data transfer, and video (alt display-port mode) so that all components can daisy chain and be controlled by the controller module.
